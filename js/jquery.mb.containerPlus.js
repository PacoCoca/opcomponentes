	...
	jQuery.containerize={
		...
		defaultButtons: {
			close : {
				...
				label:"&#x2714;",
				className:"",
				action: function(el){jQuery(el).containerize("close",100)}
			},
			...
			dock : {
				...
				action: function(el, btn){
					...
					//jQuery(el).containerize("collapse", dock);
...
//					el.$.containerize("setstate");
			...
			iconize:function(dockId){
				...
				el.$.animate({top:t,left:l,width:0,height:0,opacity:0},isAtInit ? 0 :el.opt.effectDuration, function(){
					...
					if(el.$.data("icon")){
						var imgIco = jQuery("<img/>").attr("src", el.$.data("icon")).addClass("icon");
						var imgIco2 = imgIco.attr("title", el.$.find("h2:first").text());
						text = imgIco2;
					}
					...
					if(!existDock) {
						...
						//var title = jQuery("<span/>").addClass("containerDocked").html(text);

						//el.iconElement.append(title);
						jQuery("#dock").append(el.iconElement);
						//el.iconElement = jQuery("<span/>").addClass("containerDocked").html(text);
